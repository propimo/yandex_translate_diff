...
class YandexTranslator
  ...
  def update_translation(original_past, original_new, translated_past, format: 'plain')
    ...
    (0..diff_size).each do |diff_index|
      ...
      deleted = delete_sentence(diff_index, left_diff, eng, index)
      added = add_sentence(diff_index, right_diff, eng, index)
  ...
  def delete_sentence(diff_index, left_diff, text, index)
    return false if left_diff[diff_index].blank? || left_diff[diff_index] !~ DELETED_SENTENCE_REGEXP
  ...
  def add_sentence(diff_index, right_diff, text, index, from: 'ru', to: 'en')
    return false if right_diff[diff_index].blank? || right_diff[diff_index] !~ ADDED_SENTENCE_REGEXP
